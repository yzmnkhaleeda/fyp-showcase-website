...
<div class="row">
  <div class="column middle">
    ...
    <div>
      <h1 align="center">Assign Lecturer</h1>
    </div>

    <?php
    $faculties = ["Faculty of Health & Science", "Faculty of Information & Technology", "Faculty of Business & Management"];
    foreach ($faculties as $faculty) {
        echo "<h2 align='center'>$faculty</h2>";
        echo "<table border='1' align='center' style='width: 90%; border-collapse: collapse; text-align: center;'>";
        echo "<thead><tr><th>Matrix ID</th><th>Name</th><th>Course</th><th>Supervising Lecturer</th></tr></thead>";
        echo "<tbody>";

        $sql2 = "SELECT student.*, lecturer.lectname 
                 FROM student 
                 LEFT JOIN lecturer ON student.lectid = lecturer.lectid 
                 WHERE student.stdfac = '$faculty'";
        $res2 = mysqli_query($db, $sql2) or die(mysqli_error($db));

        while ($r = mysqli_fetch_array($res2)) {
            echo "<tr>";
            echo "<td>" . $r['stdmtx'] . "</td>";
            echo "<td><a href='adminassignstd2.php?id=" . $r['stdid'] . "'>" . $r['stdname'] . "</a></td>";
            echo "<td>" . $r['stdcrs'] . "</td>";
            echo "<td>" . ($r['lectname'] ? $r['lectname'] : "") . "</td>";
            echo "</tr>";
        }

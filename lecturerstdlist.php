...
<table border="1" align="center" style="width: 90%; border-collapse: collapse; text-align: center;">
  <thead>
    <tr>
      <th>Matrix ID</th>
      <th>Name</th>
  ...
  <tbody>
    <?php
    $lectid = $_SESSION['lecturer'];
    $sql2 = "SELECT student.stdid, student.stdmtx, student.stdname, student.stdcrs, project.pjgrade
             FROM student
             LEFT JOIN project ON project.stdid = student.stdid
             WHERE student.lectid = $lectid";
    $res2 = mysqli_query($db, $sql2) or die(mysqli_error($db));

    while ($r = mysqli_fetch_array($res2)) {
        echo "<tr>";
        echo "<td>" . $r['stdmtx'] . "</td>";
        echo "<td><a href='lecturerstdlist2.php?id=" . $r['stdid'] . "'>" . $r['stdname'] . "</a></td>";
        echo "<td>" . $r['stdcrs'] . "</td>";
        echo "<td>" . ($r['pjgrade'] ? $r['pjgrade'] : "") . "</td>";
        echo "</tr>";
    }
    ?>
  </tbody>

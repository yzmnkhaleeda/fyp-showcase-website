...
<div class="topnav">
  ...
  <a href="studentregproject.php" class="active">Register Projects</a>
...
<div class="row">
  <div class="column middle">
    ...
    <div>
      <h1 align="center">Project Registration </h1>
	</div>
    
 <?php
    $sql = mysqli_query($db, "SELECT student.*, lecturer.lectname FROM student 
...
$stdid = $userRow['stdid'];
$stdic = $userRow['stdic'];
$stdname = $userRow['stdname'];
$stdmtx = $userRow['stdmtx'];
$stdcrs = $userRow['stdcrs'];
$stdfac = $userRow['stdfac'];
...
<div style="max-width: 55%; margin: 0 auto; margin-bottom: 40px;">
  <form method="post" enctype="multipart/form-data">
    ...
    <label for="lectname">Assigned Lecturer</label>
    <input type="text" id="lectname" name="lectname" value="<?php echo $lectname; ?>" readonly>

    <label for="pjtitle">Project Title</label>
    <input type="text" id="pjtitle" name="pjtitle">

    <label for="pjabstract">Project Abstract</label><br>
    <textarea id="pjabstract" name="pjabstract" rows="5" cols="100%"></textarea><br>

    <label for="pjposter">Project Poster</label>
    <input type="file" id="pjposter" name="pjposter" accept="image/*">

    <label for="pjvideo">Project Presentation</label>
    <input type="file" id="pjvideo" name="pjvideo" accept="video/*">
...
if (isset($_POST['submit'])) {
    ...
    $pjabstract = $_POST['pjabstract'];
    $pjposter = mysqli_real_escape_string($db, file_get_contents($_FILES['pjposter']['tmp_name']));
    $pjvideo  = mysqli_real_escape_string($db, file_get_contents($_FILES['pjvideo']['tmp_name']));

    $query = "INSERT INTO project (stdid, pjtitle, pjabstract, pjposter, pjvideo) 
          VALUES ('$stdid', '$pjtitle', '$pjabstract', '$pjposter', '$pjvideo')";

    if (mysqli_query($db, $query) or die(mysqli_error($db))) {
    echo "<p align='center'>Project registration successful!</p>";
    }
